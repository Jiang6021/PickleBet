...
export const MatchCard: React.FC<MatchCardProps> = ({ match, currentUser }) => {
  ...
  const [confirmingSelection, setConfirmingSelection] = useState<Prediction | null>(null);
  ...
  // Step 1: Initiate Bet (Ask for confirmation)
  const handlePreBet = (selection: Prediction) => {
    setBetError(null);
    if (betAmount <= 0 || betAmount > currentUser.balance) {
      setBetError("餘額不足或金額無效");
      return;
    }
    setConfirmingSelection(selection);
  };
  ...
  const handleConfirmBet = async () => {
    ...
    const selection = confirmingSelection;
    ...
    try {
      ...
      setConfirmingSelection(null); // Close confirmation
  ...
  const getPredictionLabel = (p: Prediction) => {
      if (p === Prediction.TEAM_A) return 'Team A';
      if (p === Prediction.TEAM_B) return 'Team B';
      if (p === Prediction.YES) return 'Yes';
      if (p === Prediction.NO) return 'No';
      return p;
  };
    ...
    <div className={`rounded-xl overflow-hidden border ${match.status === MatchStatus.FINISHED ? 'border-slate-700 bg-slate-800/50 opacity-80' : 'border-slate-600 bg-slate-800'} shadow-xl mb-6 transition-all`}>
      ...
      <div className="p-4 flex justify-between items-center relative">
        <div className="flex-1 flex flex-col items-center justify-center">
          <div className="flex flex-col gap-0.5 items-center">
             {match.teamA.map((player, idx) => (
                 <div key={idx} className="text-xl md:text-2xl font-black text-slate-100 leading-tight">
                     {player}
                 </div>
             ))}
          </div>
          <div className="text-xs text-lime-400 font-mono mt-2">Team A</div>
          {match.status === MatchStatus.FINISHED && match.markets[0].result === Prediction.TEAM_A && (
            <span className="text-lime-400 font-bold mt-1">WINNER</span>
          )}
        </div>
        
        <div className="mx-2 text-slate-500 font-thin text-xl">VS</div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          ...
          <div className="text-xs text-indigo-400 font-mono mt-2">Team B</div>
        ...
        <div className="p-4 bg-slate-900/50 border-t border-slate-700">
            {isPlayer ? (
                ...
                <>
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-2 mb-4 scrollbar-hide">
                    {match.markets.map((m, idx) => (
                      <button
                        ...
                        disabled={confirmingSelection !== null}
                  ...
                  {confirmingSelection ? (
                      <div className="bg-slate-800 border border-amber-500/50 rounded-lg p-4 animate-in fade-in zoom-in duration-200">
                          <div className="text-center mb-4">
                              <p className="text-slate-400 text-sm mb-1">您確定要下注嗎？</p>
                              <div className="text-white text-lg font-bold">
                                  ${betAmount} <span className="text-slate-400 text-sm">on</span> <span className="text-lime-400">{getPredictionLabel(confirmingSelection)}</span>
                              </div>
                          </div>
                          <div className="flex gap-3">
                              <button 
                                  onClick={handleCancelBet}
                                  className="flex-1 py-2 rounded bg-slate-700 text-slate-300 hover:bg-slate-600 font-bold transition-colors"
                              >
                                  取消
                              </button>
                              <button 
                                  ...
                                  disabled={isBetting}
                                  className="flex-1 py-2 rounded bg-lime-500 text-slate-900 hover:bg-lime-400 font-bold transition-colors shadow-[0_0_15px_rgba(132,204,22,0.4)]"
                              >
                                  {isBetting ? '處理中...' : '確認下注'}
                              </button>
                          </div>
